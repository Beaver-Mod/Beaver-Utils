...
export async function handleEmbedButtonClick(interaction: ButtonInteraction) {
  ...
  switch (action) {
    ...
    case 'json': {
      switch (subAction) {
        case 'export': {
          ...
          embedJSONData.content = interaction.message.content as any;
          embedJSONData.embeds = interaction.message.embeds as any;
          writeFileSync('data/embed.json', JSON.stringify(embedJSONData, null, 2));
          await interaction.reply({ content: 'Embed JSON', ephemeral: true, files: ['data/embed.json'] });
          unlinkSync('data/embed.json');
          break;
        }
        case 'import': {
    ...
    case 'delete': {
      switch (subAction) {
        case 'embed': {
          const embedButtons: APIButtonComponent[] = [];
          const embedButtons2: APIButtonComponent[] = [];
          ...
          if (embedButtons.length > 0)
            components.push(new ActionRowBuilder<ButtonBuilder>({ components: embedButtons }));
          if (embedButtons2.length > 0)
            components.push(new ActionRowBuilder<ButtonBuilder>({ components: embedButtons2 }));
          components.push(new ActionRowBuilder<ButtonBuilder>().addComponents(buttons.home));
          await interaction.update({ components });
          break;
        }
        case 'img': {
          const imgButtons: APIButtonComponent[] = [];
          const imgButtons2: APIButtonComponent[] = [];
          interaction.message.attachments.forEach((img) => {
            if (imgButtons.length >= 5) {
              imgButtons2.push({
                custom_id: `e.image.delete.${(interaction.message.attachments as any).indexOf(img)}`,
                label: `Image: ${(interaction.message.attachments as any).indexOf(img) + 1}`,
                style: ButtonStyle.Primary,
                type: ComponentType.Button,
              });
            } else {
          ...
          const components = [];
          components.push(new ActionRowBuilder<ButtonBuilder>({ components: imgButtons }));
          components.push(new ActionRowBuilder<ButtonBuilder>({ components: imgButtons2 }));
          components.push(new ActionRowBuilder<ButtonBuilder>().addComponents(buttons.home));
          await interaction.update({ components });
          break;
        }
    ...
    case 'embed': {
      const embedIndex = Number(interaction.customId.substring(2).split('.')[2]) || 0;
      ...
      switch (subAction) {
        ...
        case 'editFields': {
          ...
          const fieldIndex = Number(interaction.customId.substring(2).split('.')[4]) || 0;
          ...
          switch (fieldAction) {
            case 'edit': {
              await interaction.update({
                components: [
                  ...
                  new ActionRowBuilder<ButtonBuilder>().addComponents(buttons.home),
                ],
              });
              break;
            }
            case 'add': {
              embeds[embedIndex] = new EmbedBuilder(embeds[embedIndex].toJSON())
            ...
            case 'select': {
              const fieldButtons: APIButtonComponent[] = [];
              const fieldButtons2: APIButtonComponent[] = [];
              const fieldButtons3: APIButtonComponent[] = [];
              const fieldButtons4: APIButtonComponent[] = [];
              ...
              if (fieldButtons.length > 0)
                components.push(new ActionRowBuilder<ButtonBuilder>({ components: fieldButtons }));
              if (fieldButtons2.length > 0)
                components.push(new ActionRowBuilder<ButtonBuilder>({ components: fieldButtons2 }));
              if (fieldButtons3.length > 0)
                components.push(new ActionRowBuilder<ButtonBuilder>({ components: fieldButtons3 }));
              if (fieldButtons4.length > 0)
                components.push(new ActionRowBuilder<ButtonBuilder>({ components: fieldButtons4 }));
              components.push(new ActionRowBuilder<ButtonBuilder>().addComponents(buttons.home));
              await interaction.update({ components });
              break;
            }
            case 'field': {
              interaction.update({
                components: [
                  ...
                  new ActionRowBuilder<ButtonBuilder>().addComponents(embedButtons.fieldDelete, buttons.home),
                ],
              });
              break;
            }
...
export async function handleEmbedModelSubmit(interaction: ModalSubmitInteraction) {
  ...
  switch (action) {
    case 'json': {
      switch (subAction) {
        case 'import': {
          ...
          const embeds = json.embeds.map((embed: APIEmbed) => new EmbedBuilder(embed));
          if (embeds.length === 0) {
            buttons.editEmbed.setDisabled(true);
            buttons.deleteEmbed.setDisabled(true);
          }
    ...
    case 'embed': {
      const embedIndex = Number(interaction.customId.substring(2).split('.')[2]);
      const embeds = interaction.message.embeds;
      const embedButtons = {
        color: new ButtonBuilder()
          .setLabel('Color')
      ...
      switch (subAction) {
        ...
        case 'image': {
          embeds[embedIndex] = new EmbedBuilder(embeds[embedIndex].toJSON()).setImage(input).toJSON() as Embed;
          await interaction.update({ embeds });
          break;
        }
        case 'thumbnail': {
          embeds[embedIndex] = new EmbedBuilder(embeds[embedIndex].toJSON()).setThumbnail(input).toJSON() as Embed;
          await interaction.update({ embeds });
          break;
        }
        case 'editFields': {
        ...
        default: {
          (embeds[embedIndex] as any).data[subAction] = input;
          await interaction.update({ embeds });
          break;
        }
      }
